...
export default function NodeTooltip({ data, isL5, isL6 }: NodeTooltipProps) {
  ...
  const tooltipContent = (
    ...
    >
      <div
        className="bg-white/95 backdrop-blur-md rounded-xl shadow-2xl border border-gray-200 p-5 min-w-[420px] max-w-[600px]"
        style={{
          boxShadow:
      ...
      >
        ...
        <div className="mb-4 pb-4 border-b-2 border-gray-200">
          ...
          {data.l4Category && (
            <div className="mt-2 text-sm text-gray-700">
              <span className="font-semibold text-gray-800">L4:</span>{" "}
              <span className="text-blue-600">{data.l4Category}</span>
            </div>
          )}
        </div>
              ...
              <div className="mb-3">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {data.predecessors.map((pred, idx) => {
                    ...
                    const displayText =
                      isL6 && pred.includes("::") ? pred.split("::")[1] : pred;
                    return (
                      <span
                        key={idx}
                        className="inline-block px-2.5 py-1 bg-gray-100 text-xs text-gray-700 rounded-md border border-gray-300 hover:bg-gray-200 transition-colors"
                      >
                        {displayText}
                      </span>
                    );
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {data.successors.map((succ, idx) => {
                    // L6인 경우 "L5ID::L6ID"에서 L6ID만 추출
                    const displayText =
                      isL6 && succ.includes("::") ? succ.split("::")[1] : succ;
                    return (
                      <span
                        key={idx}
                        className="inline-block px-2.5 py-1 bg-gray-100 text-xs text-gray-700 rounded-md border border-gray-300 hover:bg-gray-200 transition-colors"
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {data["선행 L5"].map((l5, idx) => (
                      <span
                        key={idx}
                        className="inline-block px-2.5 py-1 bg-blue-50 text-xs text-blue-700 rounded-md border border-blue-200"
                      >
                        {l5}
                      </span>
                    ))}
        ...
        {/* Warning for cycles */}
        {data.hasCycle && (
          <div className="mt-4 pt-4 bg-gradient-to-r from-red-50 to-red-100 p-3 rounded-lg border border-red-300">
