      ...
      try {
        ...
        l5Tasks.forEach((task) => {
          ...
          task.predecessors.forEach((predId) => {
            const { foundId, hasCaseMismatch } = findTaskIdCaseInsensitive(
              predId,
              l5Tasks,
            );
            if (foundId) {
              ...
              if (successor?.successors.includes(task.id)) {
                ...
                if (task.id < succId) {
                  ...
                  const hasCaseMismatch = caseMismatchErrors.some(
                    (err) =>
                      (err.sourceTask === task.id &&
                        err.relatedTask === succId) ||
                      (err.sourceTask === succId &&
                        err.relatedTask === task.id),
                  );
        ...
        l5Tasks.forEach((task) => {
          ...
          const hasSelfLoop =
            task.predecessors.includes(task.id) ||
            task.successors.includes(task.id);
        ...
        l5Tasks.forEach((task) => {
          ...
          if (task.predecessors.length === 0) {
            ...
            l5Tasks.forEach((otherTask) => {
              if (
                ...
                otherTask.successors.includes(task.id)
              ) {
                errors.push({
          ...
          if (task.successors.length === 0) {
            ...
            l5Tasks.forEach((otherTask) => {
              if (
                otherTask.id !== task.id &&
                otherTask.predecessors.includes(task.id)
              ) {
                errors.push({
