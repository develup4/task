...
export interface L5CriticalPathResult {
  path: string[]; // L5 task IDs in critical path
  totalDuration: number; // Total T in weeks
}
...
/**
 * L5 태스크들의 크리티컬 패스를 계산합니다.
 * 가장 긴 경로(T의 합이 최대)를 찾습니다.
 */
export function calculateL5CriticalPath(
  l5Tasks: L5Task[]
): L5CriticalPathResult {
  if (l5Tasks.length === 0) {
    return { path: [], totalDuration: 0 };
  }

  const taskMap = new Map(l5Tasks.map((t) => [t.id, t]));

  // 각 노드까지의 최대 경로 길이와 경로를 저장
  const maxPath = new Map<string, { duration: number; path: string[] }>();

  // 시작 노드들 찾기 (선행이 없는 노드들)
  const startNodes = l5Tasks.filter((t) => t.predecessors.length === 0);
  ...
  // 모든 시작 노드에서 시작하여 최대 경로 찾기
  // duration이 같으면 경로 길이가 더 긴 것을 선택
  let criticalPath: L5CriticalPathResult = { path: [], totalDuration: 0 };

  if (startNodes.length === 0) {
    // 시작 노드가 없으면 (모든 노드가 선행을 가짐 - 순환 구조) 모든 노드를 시도
    for (const task of l5Tasks) {
      const result = dfs(task.id, new Set());
      if (
        result.duration > criticalPath.totalDuration ||
        (result.duration === criticalPath.totalDuration &&
